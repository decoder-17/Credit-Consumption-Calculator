...
export type GraduatedTier = {
  upToCredits: number | null;
  creditsPerUnit: number;
};
export type GraduatedTiersConfig = {
  unitsPerBlock: number;
  tiers: GraduatedTier[];
};

export function graduatedTiers(
  quantity: number,
  config: GraduatedTiersConfig,
): StrategyResult {
...
export function committedWithOverage(
  quantity: number,
  committed: number,
  rate: number,
): StrategyResult {
...
export function freeTierThenOverage(
  quantity: number,
  freeQuantity: number,
  rate: number,
): StrategyResult {
